...
class BMI_USGS():
    def __init__(self):
        """Create a Bmi data ret. model that is ready for initialization."""
        ...
        self._input_var_names = [
            'site','service','start','end']
    
        #---------------------------------------------
        # Output variable names (CSDMS standard names)
        #---------------------------------------------
        self._output_var_names = ['peak_va']
        ...
        self._var_name_units_map = {
                                'stream_flow-gage':['peak_va','ft3 s-1'],
...
#         lateral_flow_linear_reservoir_constant *= 3600.0                                              # Not used
    ...
    def load_unit_test_data(self):
        ...
        self.usgs_output_data = pd.DataFrame().reindex_like(self.unit_test_data)
    ...
    def run_unit_test(self, plot_lims=list(range(490, 550)), plot=False, print_fluxes=True):
        ...
        for t, precipitation_input in enumerate(self.forcing_data['precip_rate']*3600):
            ...
            self.usgs_output_data.loc[t,'Time']      = self.current_time
            self.usgs_output_data.loc[t,'Time Step'] = self.current_time_step
            self.usgs_output_data.loc[t,'Rainfall']  = self.timestep_rainfall_input_m

            self.usgs_model.run_usgs(self)
            
            self.usgs_output_data.loc[t,'Direct Runoff']   = self.surface_runoff_depth_m
            self.usgs_output_data.loc[t,'GIUH Runoff']     = self.flux_giuh_runoff_m
            self.usgs_output_data.loc[t,'Lateral Flow']    = self.flux_nash_lateral_runoff_m
            self.usgs_output_data.loc[t,'Base Flow']       = self.flux_from_deep_gw_to_chan_m
            self.usgs_output_data.loc[t,'Total Discharge'] = self.flux_Qout_m
            self.usgs_output_data.loc[t,'Flow']            = self.total_discharge
        ...
        if plot: 
            for output_type in ['Direct Runoff', 'GIUH Runoff', 'Lateral Flow', 'Base Flow', 'Total Discharge', 'Flow']:
                plt.plot(self.usgs_output_data['Rainfall'][plot_lims], label='precipitation', c='gray', lw=.3)
                plt.plot(self.usgs_output_data[output_type][plot_lims], label='usgs '+output_type)
